...
const cartSlice = createSlice({
  ...
  reducers: {
    addItem(state, action) {
      ...
      console.log(pam.id)
      const existingItem = state.items.find(item => item.id === pam.id);
      if (!existingItem) {
        
        state.items.push({ id: pam.id,total:pam.price, price:pam.price,title:pam.title,description:pam.description,quantity:1});
